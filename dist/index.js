...
window.addEventListener("load", function () {
    ...
    request.onload = function (ev) {
        ...
        if (this.readyState !== 4 || (this.status !== 200 && this.status !== 0))
            return;
        var trackerList = this.responseText.trim().split(/\s+/);
        ...
        var _loop_1 = function (i) {
            var currentTracker = trackerList[i];
...
function ShowTrackers(show) {
    ...
    if (show) {
        var buttonContainer = document.getElementById("tracker_button_container").children;
...
function SetTorrentData() {
    ...
    if (okTrackersList.length !== 0) {
        ...
        var announceList = [];
        for (var i = 0; i < okTrackersList.length; ++i)
...
function Finished() {
    ...
    if (msSaveOrOpenBlob) {
        ...
        var a_1 = document.getElementById("download_link");
...
function DisableElements(disable) {
    function DisableInternal(elementName, isDisabled) {
        document.getElementById(elementName).disabled = isDisabled;
    }
...
function CreateFromFile(obj) {
    ...
    var maxChunkCount = Math.ceil(fileSize / readChunkSize);
    var blocksPerChunk = readChunkSize / blockSize;
    var maxBlockCount = Math.ceil(fileSize / blockSize);
    var pieces = new Uint8Array(20 * maxBlockCount);
...
var BencodeDict = /** @class */ (function (_super) {
    ...
    function BencodeDict(obj) {
        ...
        _this.data = {};
        for (var key in obj) {
            var bencodeObj = _this.getBencodeObject(obj[key]);
            if (bencodeObj)
    ...
    BencodeDict.prototype.encode = function (array) {
        ...
        for (var i = 0; i < sortedKeys.length; ++i) {
            ...
            var currentObject = this.data[currentKey];
            var bencodedKeyObject = this.getBencodeObject(currentKey);
...
var BencodeList = /** @class */ (function (_super) {
    ...
    function BencodeList(list) {
        ...
        for (var i = 0; i < list.length; ++i) {
            var bencodeObj = _this.getBencodeObject(list[i]);
            if (bencodeObj)
...
var BencodeString = /** @class */ (function (_super) {
    ...
    BencodeString.prototype.encode = function (array) {
        var bytesCount = stringByteCount(this.value);
        array.push.apply(array, toByteArray(bytesCount.toString()));
        array.push(":".charCodeAt(0));
